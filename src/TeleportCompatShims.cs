...
public static class TeleportCompatShims
{
    ...
    public static IEnumerator PlacePlayerViaCoords(Vector3 target)
    {
        ...
        // Detailed debug logging (can be enabled via TBLog.Debug)
        try { TBLog.Info($"[TeleportCompatShims] initial player position: {playerTransform.position}"); } catch {}
        ...
        {
            TBLog.Warn("PlacePlayerViaCoords: error reading components: " + ex);
        }
        ...
        try { TBLog.Info($"[TeleportCompatShims] final player position: {playerTransform.position}"); } catch {}
