...
public static class TeleportHelpers
{
    ...
    // --- helper: zkus� opravit b�n� prohozen� sou�adnic (Y<->Z) pomoc� permutac� a probe ---
    ...
    // --- hlavn� metoda s integrovanou opravou a bezpe�nostn�mi kontrolami ---
    ...
    public static bool AttemptTeleportToPositionSafe(Vector3 target)
    {
        try
        {
            ...
            // --- start replacement block: robustn� v�b�r autoritativn�ho PlayerChar rootu ---
            try
            {
    ...
    // Robustn�j�� permutace / validace coords � zkou�� origin�l a Y<->Z a vyb�r� tu, kter� d�v� rozumnou "grounded" pozici.
    // Preference: 1) NavMesh.SamplePosition (pokud dostupn�) 2) raycast grounding; v�b�r preferuje pozici uvnit� rozumn� v��ky.
    public static bool TryPickBestCoordsPermutation(Vector3 original, out Vector3 chosenSafe)
    {
        chosenSafe = original;
        try
        {
            // Rozsahy pova�ovan� za "rozumn�" pro hern� sv�t - uprav dle pot�eby (u tebe hr�� okolo -20)
            const float minReasonableY = -200f;
            const float maxReasonableY = 300f;

            // Permutace k otestov�n� (origin�l a Y<->Z)
    ...
    // Najde bezpe�n� m�sto bl�zko po�adovan� pozice:
    //  - nejd��ve zkus� NavMesh.SamplePosition (pokud je NavMesh v projektu),
    //  - pak naho�e-dolu raycast pro zji�t�n� povrchu,
    //  - nakonec vr�t� false pokud nic nen� bezpe�n�.
    ...
    public static bool TryFindNearestNavMeshOrGround(Vector3 desired, out Vector3 outPos, float navSearchRadius = 10f, float maxGroundRay = 400f)
    {
        ...
        try
        {
            ...
            // 2) Ground raycast shora dol� - pokud navmesh nenalezen
            try
            {
            ...
            // 3) Fallback: zkus�me mal� zvednut� a hled�n� nezapadaj�c�ch pozic (minim�ln� z�chrana)
            try
            {
