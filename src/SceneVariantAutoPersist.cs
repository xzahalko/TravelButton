...
public class SceneVariantAutoPersist : MonoBehaviour
{
    ...
    public static IEnumerator HandleSceneLoadedCoroutine(Scene scene)
    {
        ...
        try
        {
            ...
            try
            {
                // Update runtime city entry and persist via existing updater/persist logic
                TravelButtonVariantUpdater.UpdateCityVariantData(scene.name, normalName, destroyedName, finalVariantStr);
                TBLog.Info($"SceneVariantAutoPersist: invoked TravelButtonVariantUpdater.UpdateCityVariantData for scene='{scene.name}'.");
            }
            catch (Exception ex)
            {
                TBLog.Warn("SceneVariantAutoPersist: UpdateCityVariantData threw: " + ex);
            }
        }
        catch (Exception ex)
        {
