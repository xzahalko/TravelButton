...
public static class VisitedStore
{
    ...
    public static bool HasVisited(string cityName)
    {
        ...
        {
            TravelButtonMod.LogWarning("VisitedStore.HasVisited exception: " + ex.Message);
            return false;
        }
    }
    ...
    public static bool MarkVisited(string cityName)
    {
        ...
        {
            TravelButtonMod.LogWarning("VisitedStore.MarkVisited exception: " + ex.Message);
            return false;
        }
    }
    ...
    public static void ClearVisited()
    {
        ...
        {
            TravelButtonMod.LogWarning("VisitedStore.ClearVisited exception: " + ex.Message);
        }
    }
    ...
    private static void LoadFromDisk()
    {
        try
        {
            ...
            if (!File.Exists(path))
            {
                ...
                TravelButtonMod.LogInfo($"VisitedStore: no visited file found at '{path}'. Starting with empty set.");
                return;
            }
            ...
            {
                visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                TravelButtonMod.LogInfo($"VisitedStore: visited file '{path}' empty -> starting with empty set.");
                return;
            }
            ...
            try
            {
                ...
                if (wrapper != null && wrapper.visited != null)
                {
                    ...
                    TravelButtonMod.LogInfo($"VisitedStore: loaded {visited.Count} visited entries from '{path}'.");
                }
                else
                {
                    visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    TravelButtonMod.LogWarning($"VisitedStore: visited file '{path}' parsed to null wrapper; starting empty.");
                }
            }
            catch (Exception exJson)
            {
                // Fall back to attempting to parse a raw JSON array using a minimal approach
                TravelButtonMod.LogWarning("VisitedStore: JsonUtility.FromJson failed: " + exJson.Message + " - attempting minimal parse.");
                try
                {
                    ...
                    if (trimmed.StartsWith("[") && trimmed.EndsWith("]"))
                    {
                        ...
                        TravelButtonMod.LogInfo($"VisitedStore: minimal parse loaded {visited.Count} entries.");
                    }
                    else
                    {
                        visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                        TravelButtonMod.LogWarning("VisitedStore: visited file did not look like an array - starting empty.");
                    }
                }
                catch (Exception ex2)
                {
                    visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
                    TravelButtonMod.LogWarning("VisitedStore: fallback parse failed: " + ex2.Message);
                }
            }
        }
        catch (Exception ex)
        {
            visited = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
            TravelButtonMod.LogWarning("VisitedStore.LoadFromDisk exception: " + ex.Message);
        }
    }
    ...
    private static void SaveToDisk()
    {
        try
        {
            ...
            TravelButtonMod.LogInfo($"VisitedStore: saved {visited.Count} visited entries to '{path}'.");
        }
        catch (Exception ex)
        {
            TravelButtonMod.LogWarning("VisitedStore.SaveToDisk exception: " + ex.Message);
        }
    }
